...
{	
	...
	sd.sensor = malloc(sizeof(Sensor*)*sd.n_sensor); /*allocate space*/
...
/*****************************************
 * add four functions for register D 
 *  in plc
 * ***************************************/
/*set controller with some rules*/
void User_controller_set(struct serialsosurce *ss,int reg_number,int status)
{
	DEBUG("setting controller info to PLC\n");

	/*this function is defined in plc.c*/
	controller_set(ss, reg_number,status);
}

/*get controller info*/
void User_controller_get(struct serialsosurce *ss,int reg_number)
{
	int  status;	/*get controller's status*/

	DEBUG("get controller info from PLC\n");

	/*this function is defined in plc.c*/
	controller_get(ss,reg_number,&status);
	sleep(2);
	
	DEBUG("controller %d status is %d\n",reg_number,status);
}


/*set sensor */
void User_sensor_set(struct serialsosurce *ss,int id, int data)
{
	DEBUG("set sensor original number...\n");

	/*this function is used to set the sensor orginal value*/
	sensor_set(ss,id,data);
		
}

/*get sensor info*/
void User_sensor_get(struct serialsosurce *ss,int sen_number)
{
	int data;

	/*this function is used to get the measured value*/
	sensor_get(ss,sen_number,&data);
	sleep(1);

	DEBUG("sensor %d returned value: %d\n",sen_number,data);
}
